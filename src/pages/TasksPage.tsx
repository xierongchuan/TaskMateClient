...
export const TasksPage: React.FC = () => {
  ...
  const createDelegationMutation = useMutation({
    mutationFn: ({ taskId, toUserId, reason }: { taskId: number; toUserId: number; reason?: string }) =>
      delegationsApi.createDelegation(taskId, toUserId, reason),
    onSuccess: () => {
      showToast({ type: 'success', message: 'Запрос на делегирование отправлен' });
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
      setDelegateTask(null);
    },
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Ошибка создания запроса';
      showToast({ type: 'error', message });
    },
  });
  ...
  const acceptDelegationMutation = useMutation({
    ...
    onSuccess: async () => {
      showToast({ type: 'success', message: 'Делегирование принято' });
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
      // Обновить selectedTask
      if (selectedTask) {
        try {
          const fresh = await tasksApi.getTask(selectedTask.id);
          setSelectedTask(fresh);
        } catch { /* ignore */ }
      }
    },
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Ошибка принятия делегирования';
      showToast({ type: 'error', message });
    },
  });

  const rejectDelegationMutation = useMutation({
    mutationFn: ({ delegationId, reason }: { delegationId: number; reason: string }) =>
      delegationsApi.rejectDelegation(delegationId, reason),
    onSuccess: async () => {
      showToast({ type: 'success', message: 'Делегирование отклонено' });
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
      if (selectedTask) {
        try {
          const fresh = await tasksApi.getTask(selectedTask.id);
    ...
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Ошибка отклонения делегирования';
      showToast({ type: 'error', message });
    },
  });
  ...
  const cancelDelegationMutation = useMutation({
    ...
    onSuccess: async () => {
      showToast({ type: 'success', message: 'Запрос на делегирование отменён' });
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
      if (selectedTask) {
        try {
          const fresh = await tasksApi.getTask(selectedTask.id);
    ...
    onError: (error: unknown) => {
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Ошибка отмены делегирования';
      showToast({ type: 'error', message });
    },
  });
  ...
  return (
    <PageContainer>
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      {!permissions.canManageTasks && permissions.canCompleteAssignedTasks && permissions.userId && (
                        <TaskEmployeeActions
                          ...
                          onDelegate={permissions.canDelegateTasks ? (t) => setDelegateTask(t) : undefined}
      ...
      <TaskDetailsModal
        ...
        onAcceptDelegation={(id) => acceptDelegationMutation.mutate(id)}
        onRejectDelegation={(id, reason) => rejectDelegationMutation.mutate({ delegationId: id, reason })}
        onCancelDelegation={(id) => cancelDelegationMutation.mutate(id)}
        isDelegationLoading={acceptDelegationMutation.isPending || rejectDelegationMutation.isPending || cancelDelegationMutation.isPending}
      />
      ...
      {permissions.userId && (
        <DelegateTaskModal
          ...
          task={delegateTask}
          userId={permissions.userId}
          onSubmit={(taskId, toUserId, reason) =>
            createDelegationMutation.mutate({ taskId, toUserId, reason })
          }
